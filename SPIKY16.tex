...
Event synchronization (\citeauthor{QuianQuiroga02b}, \citeyear{QuianQuiroga02b}; cf. \citeauthor{Kreuz07c}, \citeyear{Kreuz07c} and \citeauthor{Kreuz11b}, \citeyear{Kreuz11b}) works as a coincidence detector quantifying the level of synchrony from the number of quasi-simultaneous appearances of spikes. There are two variants. While the first variant uses a fixed coincidence window $\tau$, the second variant is parameter- and scale-free, since the time lag $\tau_{ij}$ up to which two spikes $t_i^{(1)}$ and $t_j^{(2)}$ are considered to be synchronous is adapted to the local spike rates:
%
...
\end{equation}
%
Here we used the definition of the second scale-free variant, for the first variant just replace $\tau_{ij}$ with $\tau$. Then with $c(y|x)$ defined accordingly, event synchronization can be written as
%
