...
SPIKY was mainly designed to facilitate the detailed analysis of one dataset. It enables the user to switch between different representations and to zoom in on both spatial and temporal features of interest. However, it is not very suitable for a more coarse-grained analysis over many different datasets when e.g. the statistics of a certain quantity such as an average over certain time intervals should be evaluated in some kind of loop over all available datasets.

For these purposes the SPIKY-package contains a program called 'SPIKY\_loop'

'SPIKY\_loop' uses the full functionality of SPIKY (access to time instants, selective and triggered averages as well as averages over spike train groups).
