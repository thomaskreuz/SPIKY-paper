...
% GUI vs Loop
%
% Computational Cost (Memory, Speed), Comparison with other measures/implementations
...
xxx We define the ISI as going from right after the last spike to the exact time of the next spike. So the previous spike is not part of the ISI while the following is. So when you are on a spike this is the following spike and the one before is the previous spike.
...
\subsection{\label{sss:Edge-effect} Edge effect}

suggested by Conor Houghton
...
\end{equation}
%
This ISI-ratio equals $0$ for identical ISI in the two spike trains, and approaches $-1$ and $1$, respectively, if the first or the second spike train is much faster than the other. For the ISI-distance the temporal averaging analogous to Eq. 
%\ref{eq:Temporal-Average} 
...
\subsection{\label{ss:Spike-train-surrogates} Spike train surrogates}
...
%https://groups.google.com/forum/?hl=fr&fromgroups=#!searchin/briansupport/SPIKE\$20distance/briansupport/ctudp9JCeb8/Y5AGiIovh5wJ
...
We also thank Thomas Alderson, Mayte Bonilla Quintana, Hamid Charkhkar, Didier Desaintjan, Mario DiPoppa, Mahboubeh Etemadi, Marion Najac, Matthew Phillips, Eugenio Piasini, Robert Rein, Michael Schaub, Eitan Schechtman, Matthew Williams, Yunguo Yu for advice and user feedback.
