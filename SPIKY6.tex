...
Earlier versions of the codes for calculating the ISI- and the SPIKE-distance have relied on sampled dissimilarity profiles. Typically the precision is set to the sampling interval of the neuronal recording. For each measure, this resulted in a matrix of order 'number of sampled time instants s' × 'squared number of spike trains' (i.e., $\# (t_s) N^2$). For small sampling intervals and a large number of recorded spike trains (the dissimilarity profile has to be calculated and stored for each pair of spike trains) this can lead to memory problems. In SPIKY we use an optimized and more memory-efficient way of storing the data where we make use of the fact that the ISI-distance is piecewise constant and the SPIKE-distance is piecewise linear.
